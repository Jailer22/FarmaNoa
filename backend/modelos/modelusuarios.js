...
ModelUsuarios.login = function(post,callback){
    ...
    MyModel.find({email:post.email,password:post.password},{nombre:1,rol:1,id:1},(error,documento) => {
        if(error) {
...
ModelUsuarios.Activar = function(post,callback){
    ...
    MyModel.find({email:post.email,Cod_activacion:post.Cod_activacion},{estado:1},(error,documento) => {
        if(error) {
            return callback({state:false,data:error})
        }
        else{
...
ModelUsuarios.listar = function(post, callback){
    MyModel.find({},{_id:1,nombre:1,apellido:1,email:1,cedula:1,password:1,Cod_activacion:1,estado:1},(error,documentos) => {
        if(error) {
            return callback({state:false,mensaje:error})
        }
        else{
