...
CarritoController.guardar = function(request,response){
    ...
    if(post.cantidad == undefined || post.cantidad == null || post.cantidad == ""){
        response.json({state:false,mensaje:"Ingrese cantidad a comprar del producto"})
        return false
    }

    ModelCarrito.guardar (post,function(respuesta){
        if(respuesta.state == true){
            response.json({state:true,mensaje:"Producto agregado correctamente"})
        }else{
...
CarritoController.modificar = function(request,response){
    ...
    response.json({state:false,mensaje:"Ingrese el id del producto a modificar"}) 
    return false
    }
